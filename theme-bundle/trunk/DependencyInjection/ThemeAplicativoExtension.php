...
class ThemeAplicativoExtension extends Extension
{
    ...
    private function registerPieDePaginaConfiguration(array $config, ContainerBuilder $container)
    {
        if (!isset($config['titulo'])) {
            $config['titulo'] = '<img src="https://www.santafe.gob.ar/assets/app/portal/imgs/marca-footer.png" alt="Gobierno de Santa Fe" style="padding-bottom:10px;">';
        }

        if (!isset($config['domicilio'])) {
            $config['domicilio'] = 'Atención telefónica: 0800-777-0801';
        }

        if (!isset($config['telefono'])) {
            $config['telefono'] = 'Lunes a viernes de 8 a 18 hs';
        }

        if (!isset($config['copyright'])) {
            $config['copyright'] = 'Atribución-CompartirIgual 2.5 Argentina';
        }
    ...
    private function registerEncabezadoConfiguration(array $config, ContainerBuilder $container)
    {
        ...
        if (!isset($config['imagen_top_url'])) {
            $config['imagen_top_url'] = '';
        }
        ...
        $container->setParameter('theme_aplicativo.datos_aplicativo.encabezado.imagen_top_url', $config['imagen_top_url']);
